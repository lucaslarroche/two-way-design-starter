...
- `/sync-figma-to-code` — Pull latest from Figma color styles → update tokens + CSS
- `/sync-code-to-figma` — Push current tokens → rebuild plugin palette → re-run plugin in Figma
...
The markers `/* tokens:start */` and `/* tokens:end */` in `_theme.css`
...
- `npm run plugin:build` — Regenerate RGB palette in figma-plugin/code.js from tokens.json
- `npm run figma:pull` — CLI shortcut: Figma color styles → tokens.json (no AI)
- `npm run figma:push` — CLI shortcut: rebuild plugin palette (then re-run plugin in Figma)
